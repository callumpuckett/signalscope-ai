...
def test_turnstile_integration_accepts_deterministic_application_markup():
    assert smoke._turnstile_integration_valid(1, "site-key", "newsletter_signup", 1)


@pytest.mark.parametrize(
    "widget_count,site_key,action,script_count",
    [(0, "site-key", "newsletter_signup", 1), (1, "", "newsletter_signup", 1),
     (1, "site-key", "", 1), (1, "site-key", "newsletter_signup", 0)],
)
def test_turnstile_integration_rejects_missing_application_markup(
    widget_count, site_key, action, script_count
):
    assert not smoke._turnstile_integration_valid(
        widget_count, site_key, action, script_count
    )

...
{% if not request.path.startswith('/newsletter') %}
...
@media(max-width:700px){
    .public-header{padding:calc(4px + env(safe-area-inset-top)) max(14px,env(safe-area-inset-right)) 4px max(14px,env(safe-area-inset-left));}
    .public-header-inner{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;}
    .public-header .logo{width:190px;}
    .public-header .logo-img{max-width:190px;max-height:none;}
...
def stockradar_header_navigation(location="public"):
    if not has_request_context():
        with app.test_request_context("/newsletter/latest"):
            return stockradar_header_navigation(location)
...
def disclaimer_footer():
    return """
    <footer style="margin:32px auto 0;padding:18px 0 0;border-top:1px solid rgba(255,255,255,0.10);color:var(--text-muted,#94a3b8);font-size:12px;line-height:1.65;max-width:1180px;">
        <div>
        ...
        <nav aria-label="Legal and support links" style="display:flex;flex-wrap:wrap;gap:14px;margin-top:12px;">
            <a href="/how-it-works" style="color:var(--text-muted,#94a3b8);">How It Works</a>
            <a href="/privacy" style="color:var(--text-muted,#94a3b8);">Privacy</a>
            <a href="/terms" style="color:var(--text-muted,#94a3b8);">Terms</a>
            <a href="/refund-policy" style="color:var(--text-muted,#94a3b8);">Refund Policy</a>
            <a href="/risk-disclaimer" style="color:var(--text-muted,#94a3b8);">Risk Disclaimer</a>
            <a href="/feedback" style="color:var(--text-muted,#94a3b8);">Feedback</a>
            <a href="/contact" style="color:var(--text-muted,#94a3b8);">Contact</a>
        </nav>
    </footer>
    """
...
body{margin:0;background:radial-gradient(circle at 15% 0%,rgba(0,255,170,0.10),transparent 30%),linear-gradient(135deg,#08111c,#101827);color:#dbe4ee;font-family:Arial,sans-serif;min-height:100vh;}
.wrap{box-sizing:border-box;max-width:992px;margin:0 auto;padding:46px;}
...
@media(max-width:700px){:root{--font-hero:clamp(32px,9vw,38px);--font-section:22px;}.wrap{padding:24px;}h1{font-size:var(--font-hero);}.section-grid,.signal-guide{grid-template-columns:1fr;}.weekly-cta{align-items:flex-start;flex-direction:column;}}
</style>
</head>
<body>
...
def stock_universe_page():
    ...
    {{ company_identity_assets() }}
    <style>
    body{margin:0;background:radial-gradient(circle at 12% 0%,rgba(0,255,170,0.10),transparent 30%),linear-gradient(135deg,#08111c,#101827);color:#dbe4ee;font-family:Arial,sans-serif;min-height:100vh;}
    .wrap{box-sizing:border-box;max-width:1264px;margin:0 auto;padding:42px;}
    .card{background:linear-gradient(180deg,rgba(18,29,42,0.97),rgba(12,22,33,0.97));border:1px solid rgba(148,163,184,0.16);border-radius:28px;padding:30px;box-shadow:0 24px 70px rgba(0,0,0,0.30);margin-bottom:22px;}
    .kicker{color:#00ffaa;font-weight:950;text-transform:uppercase;letter-spacing:0.13em;font-size:12px;margin:0 0 10px 0;}
    ...
    th{color:#94a3b8;text-transform:uppercase;letter-spacing:0.08em;font-size:12px;}
    .muted{color:#94a3b8;}
    @media(max-width:760px){.wrap{padding:24px;}.card{overflow-x:auto;}h1{font-size:34px;}table{font-size:14px;}}
    </style>
    </head>
    <body>
    {{ stockradar_header_navigation('app') | safe }}
...
def premium_decision(symbol):
    ...
    if not premium_has_access():
        ...
        <style>
        *{box-sizing:border-box;}
        body{margin:0;background:linear-gradient(135deg,#050505,#111827);color:white;font-family:Arial,sans-serif;min-height:100vh;}
        .wrap{max-width:1012px;margin:0 auto;padding:46px;}
        ...
        @media(max-width:760px){.wrap{padding:24px 16px;}.card{padding:24px 20px;border-radius:24px;}.preview-grid{grid-template-columns:1fr;}h1{font-size:32px;}.button{display:block;text-align:center;}}
        </style>
        </head>
        <body>
        {{ stockradar_header_navigation('app') | safe }}
    ...
    *{box-sizing:border-box;}
    ...
    body{margin:0;background:radial-gradient(circle at 12% 6%,rgba(0,255,170,0.10),transparent 30%),linear-gradient(135deg,var(--navy),#101827);color:var(--text);font-family:Arial,sans-serif;min-height:100vh;}
    .wrap{max-width:1212px;margin:0 auto;padding:46px;}
    ...
    @media(max-width:900px){.wrap{padding:24px 16px;}.card,.summary-card{padding:24px 20px;border-radius:24px;}.decision-grid{grid-template-columns:repeat(2,minmax(0,1fr));}.decision-card:last-child{grid-column:1/-1;}.breakdown,.learning-grid{grid-template-columns:1fr;}}
...
.signal.buy{background:rgba(34,197,94,.14);color:#4ade80}
.signal.hold{background:rgba(245,158,11,.14);color:#f4c95d}
.signal.sell{background:rgba(239,68,68,.14);color:#fb7185}
.research-rows .research-label{color:var(--text-secondary);font-weight:600}

/* Opportunities hierarchy: consistent assessment headers and quieter supporting research. */
.grid{gap:24px}.opportunity{padding:28px}.opportunity .topline{display:grid;grid-template-columns:minmax(0,1fr) 138px;align-items:start;gap:24px}.opportunity .identity{display:flex;flex-wrap:nowrap;align-items:flex-start;gap:12px}.opportunity .rank{flex:0 0 32px;width:32px;height:32px;font-size:13px}.opportunity h3{font-size:23px;line-height:1.3}.opportunity .score{padding:0 0 0 20px;border-radius:0;border-left:1px solid rgba(148,163,184,.16);background:transparent;line-height:1.2;font-variant-numeric:tabular-nums}.score-label{margin-bottom:6px}.score-note{margin-top:4px;line-height:1.4}.signal-row{margin:20px 0;gap:10px 20px}.signal-row .status{margin-left:auto;padding:4px 0;background:transparent;font-size:10px}.signal-row .signal{font-size:12px;padding:7px 11px}.research-detail summary{font-size:14px;font-weight:700}.research-body{line-height:1.65}.research-body h4{margin:22px 0 12px}.research-rows{row-gap:12px}.snapshot-freshness{margin:8px 0 18px;color:#91a3b4;font-size:12px;letter-spacing:.03em}.snapshot-freshness time{color:#b7c5d1}@media(max-width:520px){.grid{gap:18px}.opportunity{padding:22px 16px}.opportunity .topline{grid-template-columns:minmax(0,1fr) 94px;gap:12px}.opportunity .identity{gap:8px}.opportunity .rank{flex-basis:24px;width:24px;height:24px;font-size:11px}.opportunity h3{font-size:18px;line-height:1.35}.opportunity .score{padding:0 0 0 12px;font-size:28px}.score-label{font-size:10px;letter-spacing:.02em}.score-note{font-size:10px}.signal-row{gap:10px 12px;font-size:12px}.signal-row .status{margin-left:0}.signal-row .signal{font-size:11px}}
</style></head><body>{{ stockradar_header_navigation('app') | safe }}<main class="wrap">
<a class="back" href="/">← Back to StockRadar</a><section class="hero"><div class="eyebrow">Premium daily research ranking</div><h1>StockRadar Opportunities</h1>{% if premium and snapshot.snapshot_date %}{% set months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] %}<p class="snapshot-freshness" title="Daily ranking snapshot date; individual data timestamps appear in the research.">Updated <time datetime="{{ snapshot.snapshot_date }}">{{ snapshot.snapshot_date[8:10]|int }} {{ months[(snapshot.snapshot_date[5:7]|int)-1] }}</time> · Daily</p>{% endif %}<p class="method">Five daily research opportunities ranked using a transparent, deterministic 100-point framework: signal, conviction, momentum, fundamentals, risk and valuation. Explanations describe the ranking; they do not select stocks.</p><p class="notice"><strong>Educational research only.</strong> Rankings are prompts for further investigation, not personalised advice or instructions to trade.</p></section>
{% macro return_metric(outlook) %}{% if outlook.cases %}<div class="upside-headline"><span class="eyebrow">12M Upside</span><strong class="upside-value">{{ outlook.metric }}</strong><p class="muted">Analyst consensus · {{ outlook.analyst_count }} analysts · Yahoo Finance</p><p class="muted">Potential price upside, not a guaranteed return.</p>{% if outlook.cached %}<p class="muted">Cached / delayed · reference quote {{ outlook.price_date }}</p>{% endif %}</div>{% endif %}{% endmacro %}
{% macro return_source(outlook) %}{% if outlook.cases %}<details class="research-detail methodology"><summary>How is 12M Upside calculated?</summary><div class="research-body"><p class="muted">Potential price upside = (Yahoo Finance analyst mean target / reference price − 1) × 100. Excludes dividends, fees and FX; not a guaranteed return.{% if outlook.cases %} Reference price: {{ outlook.reference }} · {{ outlook.price_date }} · {{ outlook.analyst_count }} analysts. Retrieved {{ outlook.retrieved_date }}. Targets accessed through yfinance; individual target dates and the exact horizon are not supplied, so 12M is indicative.{% else %} Reliable comparable analyst targets or a recent price are unavailable.{% endif %}</p><p class="muted">Analyst mean, high and low targets are base, bull and downside references, not probability-weighted StockRadar forecasts. The low target is not a loss floor and may be above today’s price.</p>{% if premium %}<p class="muted">Company fundamentals and earnings dates: Yahoo Finance via yfinance. Conditional observations from available figures, not a comprehensive company thesis.</p>{% endif %}</div></details>{% endif %}{% endmacro %}
{% macro opportunity_header(item, ranked=false) %}<div class="topline"><div class="identity">{% if ranked %}<span class="rank">{{ item.rank }}</span>{% endif %}<h3>{{ item.label }}</h3></div><div class="score"><span class="score-label">Decision Score</span>{{ item.opportunity_score|int }}<small>/100</small><span class="score-note">Opportunity ranking</span></div></div><div class="signal-row"><span class="signal {{ item.signal|lower }}">{{ item.signal }}</span><span>Risk: <strong>{{ item.risk }}</strong></span><span>Momentum: <strong>{{ item.momentum }}</strong></span>{% if ranked %}<span class="status {{ item.status|lower }}">{{ item.status }}</span>{% endif %}</div>{% endmacro %}
...
<style>
body{margin:0;min-height:100vh;background:linear-gradient(135deg,#08111c,#101827);color:#dbe4ee;font-family:Arial,sans-serif}.unavailable-wrap{min-height:calc(100vh - 100px);display:grid;place-items:center;padding:24px}.card{max-width:680px;padding:30px;border:1px solid rgba(148,163,184,.2);border-radius:24px;background:#121d2a}h1{color:#f2f5f8}p{line-height:1.7;color:#b9c5d2}a{color:#69c9f2;font-weight:900;text-decoration:none}
</style>
</head>
<body>
{{ stockradar_header_navigation('app') | safe }}
...
body.dashboard-view{display:block;}
...
.dashboard-view .main{box-sizing:border-box;width:min(1180px,100%);max-width:1180px;overflow:visible;}
...
<!DOCTYPE html>
<html>
<head><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Login</title><style>*{box-sizing:border-box;}body{margin:0;background:#020617;color:white;font-family:Arial;min-height:100vh;}.login-wrap{min-height:calc(100vh - 100px);display:flex;justify-content:center;align-items:center;padding:24px;}form{background:#0f172a;padding:40px;border-radius:20px;width:340px;border:1px solid rgba(255,255,255,0.08);}input{width:100%;padding:14px;margin-bottom:15px;border:none;border-radius:10px;}button{width:100%;padding:14px;background:#38bdf8;border:none;border-radius:10px;color:white;font-weight:bold;cursor:pointer;}a{color:#38bdf8;}</style></head>
<body>{{ stockradar_header_navigation('app') | safe }}<div class="login-wrap"><form method="POST"><input type="hidden" name="csrf_token" value="{{ csrf_token() }}"><h1>🔐 Login</h1><p style="color:#94a3b8;">Sign in to access your account.</p>{% if login_error %}<p style="background:rgba(239,68,68,0.16);border:1px solid rgba(239,68,68,0.35);color:#fecaca;padding:12px;border-radius:10px;font-weight:bold;">{{ login_error }}</p>{% endif %}<input type="email" name="email" placeholder="Email" maxlength="254" required><input type="password" name="password" placeholder="Password" maxlength="1024" required><button type="submit">Login</button><p style="color:#94a3b8;font-size:13px;margin-top:20px;">Sign in to continue.</p><p><a href="/">Return to Dashboard</a></p>{{ disclaimer_footer() | safe }}</form></div></body>
</html>
"""

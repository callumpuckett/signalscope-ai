...
<div class="sidebar">
    ...
    {% else %}
        ...
        <a class="nav-link" href="/login">🔐 Owner Login</a>
    {% endif %}
...
{% if not has_premium_access %}<div class="example-report"><small>Premium locked</small><h2>Unlock the full {{ symbol }} Pro Intelligence report</h2><p>Free shows the basic signal, confidence score and meter. Premium unlocks the full AI confidence panel, risk read, next move and clearer decision support.</p><div class="example-report-grid"><div class="example-report-card"><strong>Free preview</strong><div class="confidence-score">{{ ai_context.confidence }}</div><div class="confidence-meter">{{ ai_context.confidence_meter }}</div><span class="strength-pill">Basic signal strength: {{ ai_context.strength_label }}</span></div><div class="example-report-card"><strong>Premium risk read</strong><span>Locked until upgrade.</span></div><div class="example-report-card"><strong>Premium next move</strong><span>Locked until upgrade.</span></div></div><a class="payment-button" href="/upgrade" style="margin-top:18px;">Upgrade to Premium — £5/month</a><div class="payment-note">Owner login unlocks premium automatically for testing.</div></div>{% endif %}
...
def login():
    ...
    if request.method == "POST":
        ...
        if not owner_login_configured():
            login_error = "Owner login is not configured. Set SIGNALSCOPE_OWNER_EMAIL and SIGNALSCOPE_OWNER_PASSWORD in your environment."

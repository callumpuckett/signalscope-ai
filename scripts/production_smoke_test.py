...
def _turnstile_integration_valid(widget_count, site_key, action, script_count):
    return (
        widget_count == 1
        and bool(str(site_key or "").strip())
...
def _desktop_smoke(browser: Browser, base_url: str, timeout_ms: int, report: SmokeReport) -> None:
    ...
    try:
        ...
        if _origin(base_url) == PRODUCTION_URL:
            ...
            if not _turnstile_integration_valid(
                turnstile.count(),
                turnstile.get_attribute("data-sitekey"),
                turnstile.get_attribute("data-action"),
                page.locator('script[src*="challenges.cloudflare.com/turnstile"]').count(),
            ):
                raise SmokeFailure("production Turnstile integration markup is incomplete")
